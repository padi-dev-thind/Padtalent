...
class MemoryTestController extends BaseController {
        ...
        try {
            ...
            if (test) {
                ...
                if (question_raw) {
                    ...
                    throw new HttpException(400, 'can not find question or question is answerd or question was deleted');
                }
            } else {
    ...
    async getResultRecentQuestion(
        ...
        try {
            ...
            let recentQuestion_raw = null;
            let nextQuestion_raw = null;
            if (test) {
                ...
                if (recentQuestion_raw) {
                    ...
                    //check time out
                    const isTimeout = memoryQuestionTimeout(
                        recentQuestion_raw,
                        recentQuestion.level,
                    );
                    if (isTimeout) {
                        await this.testRepository.update(
                            {
    ...
    async setTimeOut(@Req() req: AuthRequest, @Res() res: Response, next: NextFunction) {
        try {
            ...
            return this.setData({ result: test.result })
                .setMessage('set time out seccessfully')
                .responseSuccess(res);
        } catch (error) {
            return this.setData({})
                .setCode(error?.status || 500)
