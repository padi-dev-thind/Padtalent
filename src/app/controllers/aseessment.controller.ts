...
class AssessmentController extends BaseController {
            ...
            for (const type of game_types) {
                ...
                if (!game_type) {
                    throw new HttpException(
                        400,
                        'Do not have the right to create game id: ' + type,
                    );
                }
            }
    ...
    async inviteByEmail(@Req() req: AuthRequest, @Res() res: Response, next: NextFunction) {
        try {
            ...
            await this.candidateRepository.findOrCreateByCondition({where:{ email: email }, defaults:{ email: email }});
            const assessment = await this.assessmentRepository.findByCondition({
                where: { id: req.params.id , hr_id: req.hr.id},
            });
            if (!assessment) {
                throw new HttpException(400, 'not found assessment');
            }
